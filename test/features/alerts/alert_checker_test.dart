...
void main() {
  ...
  group('satisfiesTrigger (TC-ALERT-003/004/005)', () {
    test('aboveTarget: current rate equal to target triggers (TC-ALERT-003)', () {
      expect(
    ...
    test('belowTarget: current rate equal to target triggers (TC-ALERT-004)', () {
      expect(
        AlertChecker.satisfiesTrigger(
          currentRate: 105,
          targetRate: 105,
  ...
  group('checkOne — trigger and history bookkeeping (TC-ALERT-006)', () {
    ...
    test('an unresolved rate (null) never triggers and only bumps lastCheckedAt', () {
  ...
  group(
    'edge-triggered rearm sequence (TC-ALERT-007/008/013/014) — the full '
    'lifecycle across many separate check cycles',
    () {
      test(
        'TC-ALERT-007: staying in-zone across the very next check (same '
      ...
      test(
        'TC-ALERT-008: condition stays true across many separate subsequent '
        ...
        () {
          ...
          for (var hour = 1; hour <= 10; hour++) {
            ...
            expect(outcome.updatedAlert.lastTriggeredAt, first.updatedAlert.lastTriggeredAt);
      ...
      test(
        'TC-ALERT-013: the rate moving back outside the zone re-arms with '
      ...
      test(
        'TC-ALERT-014: re-entering the zone after having left it and '
